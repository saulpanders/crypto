...
# in: string (hexidecimal); out: dict with {key_byte: decrypt_attempt}
...
# in: out: dict with {key_byte: decrypt_attempt}; out: dict with {key_byte: (decrypt_attempt,fitness_score)}
...
# in: string (hex) ; out: stdout print (top 5 results)
...
def file_in(filename):
	with open(filename) as f:
		ciphertext_results = [decipher(x) for x in f]
